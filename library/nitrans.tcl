# nitrans.tcl --
#
#	Display routines for the nihilist transposition cipher type.
#
# RCS: @(#) $Id: nitrans.tcl,v 1.3 2004/09/08 17:05:00 wart Exp $
#
...
proc display_cipher_nitrans {args} {
    global cipherinfo
	...
	$t insert $row.[expr {$i+3}] { } [list nitrans_col col_$i]
    }
    $t insert $row.end \n
    incr row
    $t insert $row.0 {   }
...
proc do_sub_nitrans {args} {
    error "This option not available for nitrans ciphers"
}

proc do_shift_nitrans {column amount} {
    global cipherinfo
...
proc display_key_nitrans {w args} {
    global cipherinfo
...
proc swap_columns_nitrans {args} {
    global cipherinfo
...
proc solve_cipher_nitrans {args} {
    global cipherinfo

    $cipherinfo(object) solve
}

proc create_input_nitrans {w {bbar {}}} {
    global cipherinfo
	...
	button	$bbar.swap -text "Swap columns" -command swap_columns_nitrans
...
proc set_bindings_nitrans {wid} {
    ...
    $wid tag bind nitrans_col <B1-Motion> {
	set tagList [%W tag names @%x,%y]
	set col {}
	foreach tag $tagList {
	    if {[regexp {^col_([0-9]+)} $tag null col]} {
	...
	if {$col != $cipherinfo(sub,ct)} {
	    do_shift_nitrans $cipherinfo(sub,ct) $cipherinfo(sub,pt)
	    %W tag remove col_selected 0.0 end
	    foreach {start end} [%W tag ranges col_$col] {
		%W tag add col_selected $start
	    }
    ...
    $wid tag bind nitrans_col <ButtonRelease-1> {
	%W tag remove col_selected 0.0 end
	break
    }
...
proc save_cipher_nitrans {chanid} {
}
