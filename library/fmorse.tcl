# fmorse.tcl --
#
#	Display routines for the fractionated morse cipher type.
#
# RCS: @(#) $Id: fmorse.tcl,v 1.2 2004/09/08 17:05:00 wart Exp $
#
...
proc display_cipher_fmorse {args} {
    global cipherinfo
...
proc display_pt_fmorse {{ct {}}} {
    ...
    for {set i 0} {$i < [string length $key_ct]} {incr i 3} {
	set ct_key([string index $alphabet [expr {$i / 3}]]) [string range $key_ct $i [expr {$i+2}]]
    }
...
#    parray ct_key
	...
	foreach el [split $word {}] {
	    $t insert $row.end "[string toupper $el]"
	    incr col
	}
    }
...
proc display_key_fmorse {w args} {
    global cipherinfo
	...
	regsub ($exp) $ct {} ct
	$w insert 3.end " "
	$w insert 3.end [string index $word 2]
    }
    $w insert 3.end \n
...
proc create_input_fmorse {w args} {
    global cipherinfo
	...
	create_radio_buttons $w.pt.r cipherinfo(sub,pt) "... ..- ..x .-. .-- .-x .x. .x- .xx -.. -.- -.x --. --- --x -x. -x- -xx x.. x.- x.x x-. x-- x-x xx. xx-"
...
proc save_cipher_fmorse {chanid} {
    global cipherinfo
